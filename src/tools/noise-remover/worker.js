...
self.onmessage = async (e) => {
    ...
    if (type === 'init') {
        try {
            ...
            // يصل رابط ONNX الكسول من الصفحة المستقلة؛ لا يعتمد العامل على المحمّل القديم.
            if (!ortUrl) throw new Error('رابط محرك المعالجة غير متاح.');
            importScripts(ortUrl);

            // تتجاور ملفات WASM مع المحرك في المصدر الكسول نفسه.
            const wasmBasePath = ortUrl.substring(0, ortUrl.lastIndexOf('/') + 1);
