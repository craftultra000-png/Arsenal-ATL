...
(function () {
  ...
  function syncThemeUI() {
    syncCSSVars();
    // نقرأ من localStorage مباشرة — المصدر الوحيد للحقيقة
    // بدلاً من parent.document.body.classList لتجنب race condition بين الـ frames
    const dark = localStorage.getItem('asl_theme') !== 'light';
  ...
  window.doToggleTheme = function() {
    if (parent.toggleTheme) {
      parent.toggleTheme();
      // parent.toggleTheme() يحفظ في localStorage فوراً — نقرأ منه مباشرة بدون rAF
      syncThemeUI();
    }
  };
...
localStorage.setItem('arsenal_user_id', String(payload.sub));
  ...
  function updateSubPageUI() {
    ...
    const monthBtn = document.querySelector('.plan3-btn[onclick*="monthly"]');
    const yearBtn  = document.querySelector('.plan3-btn[onclick*="yearly"]');

    // إعادة تعيين الأزرار أولاً
    if (monthBtn) { monthBtn.disabled = false; monthBtn.style.opacity = ''; monthBtn.style.cursor = ''; }
    if (yearBtn)  { yearBtn.disabled  = false; yearBtn.style.opacity  = ''; yearBtn.style.cursor  = ''; }

    if (!active) return;

    const currentLabel = `
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
      <span>${window.t ? t('plan_current') : 'خطتك الحالية ✓'}</span>
    `;

    if (plan === 'monthly' && monthBtn) {
      monthBtn.innerHTML = currentLabel;
      monthBtn.disabled  = true;
      monthBtn.style.opacity = '0.7';
      monthBtn.style.cursor  = 'default';
    }
    if (plan === 'yearly' && yearBtn) {
      yearBtn.innerHTML = currentLabel;
      yearBtn.disabled  = true;
      yearBtn.style.opacity = '0.7';
  ...
  let currentPaymentPlan = 'monthly';

  const planMeta = {
    monthly: { price: '5$',  labelKey: 'plan_month_name', label: 'VIP شهري'  },
    yearly:  { price: '50$', labelKey: 'plan_year_name',  label: 'VIP سنوي'  }
  };

  function showModalState(state) {
    document.getElementById('modal-loading').style.display = state === 'loading' ? '' : 'none';
    document.getElementById('modal-info').style.display    = state === 'info'    ? '' : 'none';
    document.getElementById('modal-error').style.display   = state === 'error'   ? '' : 'none';
  }

  window.openPaymentModal = async function(plan) {
    const userId = localStorage.getItem('arsenal_user_id');
    if (!userId) {
      alert(window.t ? t('pay_login_required') : 'يجب تسجيل الدخول أولاً');
      return;
    }
    ...
    try {
      ...
      if (data.pay_address) {
        document.getElementById('modal-address').textContent = data.pay_address;
        document.getElementById('modal-amount').textContent  = data.pay_amount;
        showModalState('info');
        // ابدأ الـ polling للتحقق من تفعيل الاشتراك
        startPaymentPolling(userId);
      } else {
        throw new Error(data.message || 'فشل إنشاء طلب الدفع');
      }
    } catch (err) {
      const errEl = document.getElementById('modal-error-text');
  ...
  // ── Polling للتحقق من تفعيل الاشتراك ────────────────────────────
  let pollingInterval = null;

  function startPaymentPolling(userId) {
    stopPaymentPolling();
    let attempts = 0;
    const maxAttempts = 36; // 6 دقائق كحد أقصى
    ...
    pollingInterval = setInterval(async () => {
      ...
      try {
        ...
        if (data.active) {
          ...
          showSubscriptionSuccess(data.plan);
        }
      } catch (e) {}
    }, 10000); // كل 10 ثواني
  }
  ...
  function showSubscriptionSuccess(plan) {
    ...
    // استبدل محتوى الـ modal-info برسالة نجاح
    info.innerHTML = `
      <div style="text-align:center;padding:12px 0;">
        <div style="width:64px;height:64px;background:rgba(0,212,170,0.12);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--acc2)" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
        </div>
        <h3 style="font-size:18px;font-weight:700;color:var(--c1);margin:0 0 8px;">${window.t ? t('sub_success_title') : '🎉 تم تفعيل اشتراكك!'}</h3>
        <p style="font-size:13px;color:var(--c2);margin:0 0 20px;">${window.t ? t('sub_success_desc') : 'مرحباً بك في Arsenal VIP'}</p>
        <button onclick="closePaymentModal(); location.reload();" style="padding:12px 28px;background:var(--acc2);border:none;border-radius:10px;color:#000;font-family:'Tajawal',sans-serif;font-size:14px;font-weight:700;cursor:pointer;">
          ${window.t ? t('sub_success_btn') : 'ابدأ الاستخدام'}
        </button>
      </div>
    `;
  ...
  window.retryPayment = function() {
    openPaymentModal(currentPaymentPlan);
  };
  ...
  window.copyModalAddress = function() {
    ...
    navigator.clipboard.writeText(addr).then(() => {
      ...
      if (!btn) return;
      const orig = btn.innerHTML;
      btn.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg><span>${window.t ? t('wallet_copied') : '✓ تم النسخ'}</span>`;
  ...
  // ── فحص الكاش عند التحميل ──────────────────
  async function checkOfflineCacheStatus() {
    const btn     = document.getElementById('offline-download-btn');
    const success = document.getElementById('offline-success');
    const desc    = document.getElementById('offline-status-desc');
    ...
    try {
      ...
      if (keys.length > 10) {
        ...
        btn.style.display     = 'none';
        if (desc) desc.textContent = window.t
          ? t('settings_offline_ready_desc')
  ...
  window.addEventListener('load', () => {
    ...
    if (savedUserId) checkSubscription(savedUserId).then(updateSubPageUI);
    else updateSubPageUI();
