...
(() => {
    ...
    // ==========================================
    // 1. أدوات ضغط الـ payload (CompressionStream)
    // ==========================================
    ...
    class QRManager {
        generateQR(elementId, text) {
            ...
            new QRCode(el, { text: text, width: 350, height: 280, colorDark: "#000", colorLight: "#fff", correctLevel: QRCode.CorrectLevel.L });
        }
        ...
        async initScanner() {
            ...
            // فحص صلاحية الكاميرا مسبقاً عبر permissions API (بدون فتحها فعلياً)
            // هذا أأمن من getUserMedia مباشرة لأنه لا يفتح stream ثم يغلقه
            if (navigator.permissions) {
                ...
                } catch (_) { /* permissions API غير مدعوم — نكمل بدونه */ }
            }
    ...
    class FileSender {
        ...
        async sendFile(file) {
            ...
            try {
                ...
                this.dc.send(JSON.stringify({ type: 'meta', name: file.name, size: rawBuffer.byteLength, mime: file.type }));

                const startTime = performance.now();
                let offset = 0;
                const totalSize = rawBuffer.byteLength;
                ...
                const chunks = await this.splitFileIntoChunks(rawBuffer, this.chunkSize);
    ...
    class FileReceiver {
        ...
        async saveFile() {
            try {
                ...
                const blob = new Blob([combined], { type: this.incomingMeta.mime || 'application/octet-stream' });
                ...
                uiManager.log(t('share_log_send_fail'), "error");
                toast.show(t('share_log_send_fail'), 'error', 5000);
                this.cleanup();
            }
        }
    ...
    class TransferManager {
        ...
        async startHosting(isRetry = false) {
            ...
            const channel = this.pc.createDataChannel('arsenal-transfer');
            ...
            const finalizeOffer = async () => {
                ...
                // ضغط الـ SDP لتصغير الـ QR payload
                const offerCode = await compressToBase64(JSON.stringify(this.pc.localDescription));
        ...
        async hostAcceptAnswer() {
            ...
            try {
                const desc = new RTCSessionDescription(JSON.parse(atob(answerCode)));
        ...
        async guestAcceptOffer(isRetry = false, forcedOfferCode = null) {
            ...
            try {
                const decompressed = await decompressFromBase64(offerCode);
