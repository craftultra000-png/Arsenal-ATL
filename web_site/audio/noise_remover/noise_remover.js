...
function initWorker() {
    ...
    worker.onmessage = ({ data: e }) => {
        switch (e.type) {
            ...
            case 'error':
                modelSection.hidden = true;
                alert(t('nrem_err_processing') + e.data);
    ...
    worker.onerror = (err) => {
        modelSection.hidden = true;
        alert(t('nrem_err_worker') + err.message);
        progressWrap.hidden = true;
    };
}
...
async function getModelBytes() {
    ...
    try {
        ...
        if (cached) {
            ...
            modelPct.textContent  = t('nrem_model_cached');
...
async function loadModel() {
    ...
    try {
        ...
        modelSection.hidden = true;
        alert(t('nrem_err_model') + err.message);
        startWrap.style.display = 'block';
        startBtn.disabled = false;
    }
}
...
async function processAudioFile(file) {
    ...
    fileMeta.textContent   = t('nrem_file_meta').replace('{name}', file.name).replace('{size}', (file.size / 1024).toFixed(1));
    progressWrap.hidden    = false;
    players.hidden         = true;
    actionRow.hidden       = true;
    podcastControls.hidden = true;
    updateUIProgress(0, t('nrem_progress_start'));
    ...
    setTimeout(async () => {
        try {
            ...
            updateUIProgress(10, t('nrem_progress_send'));
            worker.postMessage({ type: 'process', data: audio }, [audio.buffer]);

        } catch (err) {
            alert(t('nrem_err_file') + err.message);
            progressWrap.hidden = true;
        }
    }, 50);
}
...
async function handleProcessDone(enhanced) {
    setStep(pstepEncode);
    updateUIProgress(95, t('nrem_progress_final'));

    setTimeout(async () => {
        cleanSamples  = enhanced;
        enhancedBlob  = await encodeWav(enhanced, SR);

        updateUIProgress(100, t('nrem_progress_done'));
...
recordBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        mediaRecorder.onstop = () => {
            ...
            const file = new File([blob], t('nrem_record_file'), { type: 'audio/webm' });
            isRecording = false;
            recordBtn.classList.remove('recording');
            recordLabel.textContent = t('nrem_btn_record');
            recordHint.textContent  = t('nrem_record_start');
            selectFile(file);
        };
        ...
        recordLabel.textContent = t('nrem_recording');
        recordHint.textContent  = t('nrem_record_stop');

    } catch (err) {
        alert(t('nrem_err_mic') + err.message);
    }
});
...
resetBtn.addEventListener('click', () => {
    ...
    pendingFile            = null;
    startWrap.style.display = 'none';
    dropText.textContent   = t('nrem_drop_default');
    podcastEnabled         = false;
    podcastToggle.setAttribute('aria-pressed', 'false');
    podcastToggle.querySelector('.nrem-toggle-text').textContent = t('nrem_off');
    currentPosition = 0;
    duration        = 0;
    setTimelinePercent(0);
    currentTimeText.textContent = '00:00';
...
podcastToggle.addEventListener('click', async () => {
    ...
    podcastToggle.querySelector('.nrem-toggle-text').textContent = podcastEnabled ? t('nrem_on') : t('nrem_off');
