...
// القص
let cutStart = 0;      // نسبة 0-1
let cutEnd   = 1;      // نسبة 0-1
let activeCutThumb = null;
...
// Cut DOM
const cutThumbStart      = document.getElementById('cutThumbStart');
const cutThumbEnd        = document.getElementById('cutThumbEnd');
const cutFill            = document.getElementById('cutFill');
const cutTimesRow        = document.getElementById('cutTimesRow');
const cutStartBadge      = document.getElementById('cutStartBadge');
const cutEndBadge        = document.getElementById('cutEndBadge');
const cutDurBadge        = document.getElementById('cutDurBadge');

// Worker Progress DOM
const workerProgressSection = document.getElementById('workerProgressSection');
const workerProgressLabel   = document.getElementById('workerProgressLabel');
const workerProgressPct     = document.getElementById('workerProgressPct');
const workerProgressFill    = document.getElementById('workerProgressFill');
...
fileInput.addEventListener('change', async (e) => {
    ...
    try {
        ...
        updateSpeedUI(1.0);
        syncReverbUI();
        cutTimesRow.style.display = 'flex';
        updateCutUI();
...
function startPlayback() {
    const startSec = cutStart * audioBuffer.duration;
    player.playbackRate = parseFloat(speedSlider.value);
    player.start(0, startSec);
...
function updateCutUI() {
    const startPct = cutStart * 100;
    const endPct   = cutEnd   * 100;

    cutThumbStart.style.left = startPct + '%';
    cutThumbEnd.style.left   = endPct   + '%';
    cutFill.style.left       = startPct + '%';
    cutFill.style.width      = (endPct - startPct) + '%';

    if (!audioBuffer) return;
    const dur       = audioBuffer.duration;
    const startSec  = cutStart * dur;
    const endSec    = cutEnd   * dur;
    const durSec    = endSec - startSec;

    cutStartBadge.innerText = 'البداية: ' + formatTime(startSec);
    cutEndBadge.innerText   = 'النهاية: ' + formatTime(endSec);
    cutDurBadge.innerText   = 'المدة: '   + formatTime(durSec);
}

// سحب thumb القص
function getCutPercent(e) {
    const rect    = progressTrack.getBoundingClientRect();
    const clientX = e.touches ? e.touches[0].clientX : e.clientX;
    return Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
}

cutThumbStart.addEventListener('pointerdown', (e) => {
    e.stopPropagation();
    activeCutThumb = 'start';
    cutThumbStart.setPointerCapture(e.pointerId);
});

cutThumbEnd.addEventListener('pointerdown', (e) => {
    e.stopPropagation();
    activeCutThumb = 'end';
    cutThumbEnd.setPointerCapture(e.pointerId);
});

document.addEventListener('pointermove', (e) => {
    if (!activeCutThumb || !audioBuffer) return;
    const pct = getCutPercent(e);
    if (activeCutThumb === 'start') {
        cutStart = Math.min(pct, cutEnd - 0.01);
    } else {
        cutEnd = Math.max(pct, cutStart + 0.01);
    }
    updateCutUI();
});

document.addEventListener('pointerup', () => {
    if (!activeCutThumb) return;
    activeCutThumb = null;
    // إذا كان يشغل — إعادة التشغيل من البداية الجديدة
    if (isPlaying) {
        player.stop();
        currentPosition = cutStart * audioBuffer.duration;
...
function startTimelineLoop() {
    ...
    timelineInterval = setInterval(() => {
        ...
        const endSec = cutEnd * audioBuffer.duration;
        if (currentPosition >= endSec || currentPosition >= audioBuffer.duration) {
            currentPosition = cutStart * audioBuffer.duration;
            stopPlayback();
            return;
        }
...
// ── 7. التصدير عبر Worker ───────────────────────────────────────
function getWorkerUrl() {
    const s = [...document.querySelectorAll('script[src]')]
        .find(s => s.src && s.src.includes('audio_rate_worker.js'));
    return s ? s.src : null;
}

function renderViaWorker(format) {
    return new Promise((resolve, reject) => {
        const workerUrl = getWorkerUrl();
        if (!workerUrl) {
            // fallback للـ main thread إذا ما في worker
            renderAudioOfflineFallback().then(resolve).catch(reject);
            return;
        }
        ...
        workerProgressSection.style.display = 'block';
        workerProgressFill.style.width = '0%';

        exportWorker.onmessage = (e) => {
            const { type, pct, label, buffer, error } = e.data;

            if (type === 'progress') {
                workerProgressLabel.innerText = label || 'جاري المعالجة...';
                workerProgressPct.innerText   = (pct || 0) + '%';
                workerProgressFill.style.width = (pct || 0) + '%';
            }

            if (type === 'done') {
                workerProgressSection.style.display = 'none';
                exportWorker.terminate(); exportWorker = null;
                // إعادة بناء AudioBuffer من الـ channels
                const { channels: chs, sampleRate: sr, length: len, numberOfChannels: numCh } = buffer;
                const ctx    = new AudioContext();
                const outBuf = ctx.createBuffer(numCh, len, sr);
                for (let c = 0; c < numCh; c++) {
                    outBuf.copyToChannel(new Float32Array(chs[c]), c);
                }
                ctx.close();
                resolve(outBuf);
            }
        ...
        for (let c = 0; c < numCh; c++) {
            channels.push(audioBuffer.getChannelData(c).slice(
                Math.floor(startSec * audioBuffer.sampleRate),
                Math.floor(endSec   * audioBuffer.sampleRate)
            ));
        }
...
// Fallback — main thread إذا ما في worker
async function renderAudioOfflineFallback() {
    const startSec = cutStart * audioBuffer.duration;
    const endSec   = cutEnd   * audioBuffer.duration;
    const rate     = parseFloat(speedSlider.value);
    const duration = (endSec - startSec) / rate;
    ...
    return await Tone.Offline(async () => {
        ...
        recPlayer.playbackRate = rate;
        recPlayer.start(0, startSec);
    }, duration);
}
...
downloadWavBtn.addEventListener('click', async () => {
    ...
    try {
        const buffer   = await renderViaWorker('wav');
...
downloadMp3Btn.addEventListener('click', async () => {
    ...
    setTimeout(async () => {
        try {
            const buffer  = await renderViaWorker('mp3');
